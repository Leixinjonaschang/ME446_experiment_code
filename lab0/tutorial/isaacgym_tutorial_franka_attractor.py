...
# Load franka asset
asset_root = "/home/phi/CLX/projects/misc/me446_lab_material/ME446_experiment_code/lab0/asset"
...
# store the trajectory of the attractor
...
def update_franka(t):
    ...
    for i in range(num_envs):
        ...
        if len(trajectories[i]) > 100: # only keep the latest 100 points
        ...
        if len(trajectories[i]) > 1:
            ...
            for j in range(len(trajectories[i]) - 1):
                p1 = trajectories[i][j]
                p2 = trajectories[i][j+1]
                verts.append([p1[0], p1[1], p1[2]])
                verts.append([p2[0], p2[1], p2[2]])
            
            # draw the trajectory line in yellow
