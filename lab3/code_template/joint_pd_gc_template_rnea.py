...
def pd_controller(model, data, q_target, kp, kd):
    ...
    # COMPLETED: compute position error and velocity error
    error = q_target - q_current
    error_dot = -v_current

    # COMPLETED: PD control law: tau_pd_raw = Kp * error + Kd * error_dot
...
def compute_gravity_torque_rnea(model, data):
    """
    Compute g(q) using Recursive Newton-Euler Algorithm (RNEA).
    
    Following the Lab Manual:
    1. Initialization: Set Joint Velocity qdot = 0, Joint Acceleration qddot = 0.
       (Linear Acceleration of Base a0 = -g is handled internally by MuJoCo's mj_rne).
    2. RNEA Calculation: Evaluate inverse dynamics to get tau = g(q).
    """
    # ---- save state ----
    qpos_save = data.qpos.copy()
    qvel_save = data.qvel.copy()
    ...
    # ---- 1. Initialization: Set joint velocity and acceleration to zero ----
    data.qvel[:] = 0.0
    data.qacc[:] = 0.0
    data.ctrl[:] = 0.0
    data.qfrc_applied[:] = 0.0
    ...
    # Sync kinematics for the current qpos
    mujoco.mj_forward(model, data)

    # ---- 2. RNEA Calculation: Get gravity compensation torque ----
    rnea_result = np.zeros(model.nv, dtype=np.float64)
    # flg_acc=0 ignores the M(q)qacc term, leaving C(q,v)v + g(q).
    # Since v=0, this returns g(q) exactly.
...
def pd_gc_controller(model, data, q_target, kp, kd):
    ...
    # COMPLETED: compute gravity torque using the RNEA method per lab manual
    tau_g = compute_gravity_torque_rnea(model, data)

    # COMPLETED: total torque = PD torque + gravity compensation torque
...
def prepare_model(model):
    """
    Configure actuators to be pure torque sources by removing gain/bias.
    """
    for i in range(model.nu):
        ...
        model.actuator_biastype[i] = 0  # mjBIAS_NONE
...
def main():
    ...
    # CRITICAL: Prepare model (remove actuator bias) BEFORE verification
    prepare_model(model)

    # ---- Verification: Compare M-matrix method vs RNEA method ----
    if controller_mode == "pd_gc":
        data_verify = mujoco.MjData(model)
        # Set to home pose for consistent check
        key_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_KEY, "home")
        mujoco.mj_resetDataKeyframe(model, data_verify, key_id)
        mujoco.mj_forward(model, data_verify)

        g_m_matrix = compute_gravity_torque(model, data_verify)
        # g_rnea = compute_gravity_torque_rnea(model, data_verify)
        
        # diff = np.abs(g_m_matrix - g_rnea)
        # print("\n" + "="*60)
        # print("Gravity Torque Computation Verification (at Home Pose):")
        # print(f"M-Matrix Method: {np.round(g_m_matrix, 3)}")
        # print(f"RNEA Method:     {np.round(g_rnea, 3)}")
        # print(f"Max Difference:  {np.max(diff):.2e}")
        # if np.max(diff) < 1e-6:
        #     print("STATUS: VERIFIED (Both methods match!)")
        # else:
        #     print("STATUS: FAILED (Significant difference detected)")
        # print("="*60 + "\n")
