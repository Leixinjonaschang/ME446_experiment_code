...
def compute_gravity_torque_rnea(model, data):
    ...
    # COMPLETED: set qvel and qacc to zero for the static case
    data.qvel[:] = 0.0
    data.qacc[:] = 0.0
    data.ctrl[:] = 0.0
    data.qfrc_applied[:] = 0.0
    ...
    # COMPLETED: call mj_rne with flg_acc=0 to get gravity term only (no M*qacc)
    rnea_result = np.zeros(model.nv, dtype=np.float64)
    flg_acc = 0
    mujoco.mj_rne(model, data, flg_acc, rnea_result)  # replace ... with flg_acc value

    # COMPLETED: extract the gravity torque for the first ARM_DOF joints
...
def pd_gc_controller(model, data, q_target, kp, kd):
    ...
    # COMPLETED: compute gravity torque using the RNEA static method
    # tau_g = compute_gravity_torque_rnea(model, data)  # or compute_gravity_torque(model, data)
