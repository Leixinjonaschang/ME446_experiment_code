...
def task_inv_dyna_controller(model, data, ee_body_id,
    ...
    """
    # ── Step 0: read current state (provided) ──
    x     = get_ee_position(data, ee_body_id)       # EE position       (3,)
    J     = get_jacobian(model, data, ee_body_id)   # Jacobian J(q)     (3 × ARM_DOF)
    q_dot = data.qvel[:ARM_DOF]                     # joint velocity    (ARM_DOF,)

    # TODO 1 ── Compute EE velocity ──
    # Hint: ẋ = J(q) q̇
    x_dot = None  # <-- replace

    # TODO 2 ── Compute commanded task-space acceleration ──
    # Hint: ẍ_cmd = ẍ_d + Kp (x_d − x) + Kd (ẋ_d − ẋ)
    #       First compute x_error = x_d − x, x_dot_error = ẋ_d − ẋ
    x_error    = None  # <-- replace
    x_ddot_cmd = None  # <-- replace

    # TODO 3 ── Compute J̇ q̇ via finite difference ──
    # Hint: J̇ ≈ (J_current − J_prev) / dt   (use np.zeros_like(J) if J_prev is None)
    #       J̇ q̇ = J̇ @ q̇
    J_dot_q_dot = None  # <-- replace

    # ── commanded joint acceleration ──
    # TODO 4 ── Compute commanded joint acceleration ──
    # q̈_cmd = J† (ẍ_cmd − J̇ q̇)
    J_pinv     = np.linalg.pinv(J)
    q_ddot_cmd = None

    # TODO 5 ── Compute torque via inverse dynamics ──
    # Hint: τ = M(q) q̈_cmd + C(q,q̇)q̇ + g(q)
    #       Use compute_dynamics(model, data) which returns (M, c_g)
    #       where c_g = C(q,q̇)q̇ + g(q)
    tau_raw = None  # <-- replace
